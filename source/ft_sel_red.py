...
def ft_red_select(x, y, choice, no_normalize, dis_kept_features, num_features=30):
    """
    ...
    if choice == "pca":
        ...
        clf.fit(x, y_labels)
        sfm = SelectFromModel(clf, threshold=-np.inf, max_features=num_features)
...
def main():
    ...
    for full_file_name in full_file_names:
        ...
        if not args.combine_files:
            ...
            try:
                new_x, new_y = ft_red_select(x, y, choice, args.no_normalize, args.dis_kept_features, args.num_features) \
                    if args.num_features else ft_red_select(x, y, choice, args.no_normalize, args.dis_kept_features)
            except ValueError:
    ...
    if args.combine_files:
        ...
        x_total, y_total = ft_red_select(x_combine_files, y_combine_files, choice, args.no_normalize, args.dis_kept_features, args.num_features) \
            if args.num_features else ft_red_select(x_combine_files, y_combine_files, choice, args.no_normalize, args.dis_kept_features)
