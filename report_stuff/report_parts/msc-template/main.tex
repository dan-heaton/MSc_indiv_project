...
\newcommand\pythonstyle{\lstset{
  ...
  emph={[4]for,if,elif,else,while,in,def,class}, emphstyle={[4]\color{blue}},
...
\begin{itemize}
	...
	\end{itemize}
	\item Look into how possible it is to build models that generalise well to new subjects.
	\begin{itemize}
		\item Along with the already-mentioned system performance on new subjects, we also saw promising results in training models on files of subjects’ initial NSAA assessments (‘V1’ files, e.g. ‘D4\_sensorMagneticField.csv’) and assessing them on follow-up assessments done 6-months later (‘V2’ files, e.g. ‘D4V2\_sensorMagneticField.csv’), as while we didn’t have the true corresponding scores for the ‘V2’ files the models did predict an average decrease in overall NSAA score per subject of 4.89 over the 6-month period, which is a realistic and expected decrease that therefore indicates potential for the system to be used as a monitoring tool for the progress of the DMD condition of subjects given the necessary data files of future versions of the subjects.
	\end{itemize}
\end{itemize}

\quad The system thus holds the potential to be directly applicable for those working with and assessing DMD subjects, which may help improve the lives of those living with the condition and possibly aid in potential breakthroughs to be made in DMD research, whilst also having great potential to be taken further to possibly be built into other systems that assess other conditions in an equally promising way in the future.
...
\section{'assess\_nsaa\_nmb\_file.py'}
\pythonexternal{code/assessnsaanmbfile.py}
\section{'comp\_stat\_vals.py'}
\pythonexternal{code/compstatvals.py}
\section{'data\_balancer.py'}
\pythonexternal{code/databalancer.py}
\section{'dis\_3d\_pos.py'}
\pythonexternal{code/dis3dpos.py}
\section{'ext\_raw\_measures.py'}
\pythonexternal{code/extrawmeasures.py}
\section{'file\_mover.py'}
\pythonexternal{code/filemover.py}
\section{'file\_renamer.py'}
\pythonexternal{code/filerenamer.py}
\section{'ft\_sel\_red.py'}
\pythonexternal{code/ftselred.py}
\section{'graph\_creator.py'}
\pythonexternal{code/graphcreator.py}
\section{'mat\_act\_div.py'}
\pythonexternal{code/matactdiv.py}
\section{'model\_predictor.py'}
\pythonexternal{code/modelpredictor.py}
\section{'predictions\_selector.py'}
\pythonexternal{code/predictionsselector.py}
\section{'rnn.py'}
\pythonexternal{code/rnn.py}
\section{'settings.py'}
\pythonexternal{code/settings.py}
\section{'test\_altdirs.py'}
\pythonexternal{code/testaltdirs.py}
...
\begin{itemize}
	\item \underline{\textbf{AD}}: All data
	\item \underline{\textbf{BPTT}}: Backpropagation through time
	\item \underline{\textbf{CRF}}: Conditional random field
	\item \underline{\textbf{D}}: Duchenne (subject)
	\item \underline{\textbf{DC}}: Data cube
	\item \underline{\textbf{DMD}}: Duchenne muscular dystrophy
	\item \underline{\textbf{ES}}: Experiment set
	\item \underline{\textbf{FFNN}}: Feedforward neural network
	\item \underline{\textbf{FFT}}: Fast Fourier transform
	\item \underline{\textbf{FRC}}: Feature-reduced via concatenation
	\item \underline{\textbf{HC}}: Healthy control (subject)
	\item \underline{\textbf{GOSH}}: Great Ormond Street Hospital
	\item \underline{\textbf{HMM}}: Hidden Markov model
	\item \underline{\textbf{JA}}: Joint angle
	\item \underline{\textbf{LSTM}}: Long short-term memory
	\item \underline{\textbf{MAE}}: Mean average error
	\item \underline{\textbf{MD}}: Muscular dystrophy
	\item \underline{\textbf{MPS}}: Model predictions set
	\item \underline{\textbf{MSE}}: Mean squared error
	\item \underline{\textbf{NMB}}: Natural movement behaviour
	\item \underline{\textbf{NSAA}}: North Star Ambulatory Assessment
	\item \underline{\textbf{PCA}}: Principal component analysis
	\item \underline{\textbf{RMSE}}: Root mean squared error
	\item \underline{\textbf{RNN}}: Recurrent neural network
\end{itemize}
